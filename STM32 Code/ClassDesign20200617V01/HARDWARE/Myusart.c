...
u8 SendFaceRecognitionRequest(USART_TypeDef *USARTx,u8 ResendTimes)
{
	...
	while(--ResendTimes)
	{
		...
		while(AckTimeoutCount<10)//【测试用5s，方便串口发送应答信息，最后将该值该小】
		{
...
//				return 0;//测试用
		...
		RequestReceive=0;
		while(RequestTimeoutCount<10)//【测试用5s，方便串口发送应答信息，最后将该值该小】
		{
...
//			OLED_ShowString(0,6,StuNumber);
//			printf("receive STU");//测试用，正式须注释
			break;
		}
		default:
//			printf("receive others");//测试用，正式须注释
			break;
	}	
}
...
//	data[1]=(u8)Temp%256;
	...
	data[0]=(u8)(Temp>>8);
	data[1]=(u8)(Temp & 0x00FF);
	
	sendlen = DataPacking(2,Stm32ID,RaspbrryID,0x01,TemperatureUpload,data,sendstr);
	ResendTimes++;
	while(--ResendTimes)
	{
		AckTimeoutEnable=1;//启动应答超时计数
