...
2020/06/23/14:33
	之前系统联调，发现扫描到非小组成员IC卡OLED姓名栏显示乱码，现做修改，显示提示信息：NameError!
...
void OLED_DisplayUpdata(u8 Name,char *IDcard,char * StudentID)
{
	...
	//显示中文姓名
	if(Name<25)//是小组成员才显示姓名
	{
		OLED_ShowCHinese(54,0,Name);
		OLED_ShowCHinese(72,0,Name+1);
		if(ChineseName!=BC)//边策名字只有两个字，就放到末尾了，其他人需要显示第三个字
			OLED_ShowCHinese(90,0,Name+2);
	}
	else
		OLED_ShowString(45,0,"NameError!");//提示当前卡号非小组成员
