...
void OLED_DisplayInit(void)
{
		...
		state=strcmp(MyText,"0F23660A");//成员1：廖廷春
		if(state==0)
		{
		...
		state=strcmp(MyText,"F0169A83");//成员2：周嘉辉
		if(state==0)
		{
		...
		state=strcmp(MyText,"C5F2241C");//成员3：边策
		if(state==0)
		{
		...
		state=strcmp(MyText,"20D72F68");//成员4：王天晖
		if(state==0)
		{
		...
		state=strcmp(MyText,"84F724CA");//成员5：胡凌桥
		if(state==0)
		{
		...
		state=strcmp(MyText,"0F237704");//成员6：毛梓蒙
		if(state==0)
		{
		...
		state=strcmp(MyText,"19189198");//成员7：测试成员
		if(state==0)
		{
		...
		OLED_ShowString(48,0,"NameError!");	//更新现实IC卡号
